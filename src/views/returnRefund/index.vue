<template>
  <section>
    ...
    <!--退款说明-->
     <ul class="theList">
        ...
        <li class="clearfix">
            ...
            <li class="clearfix">
            <label class="fl" for="remark">退款说明</label>
            <input class="fr" id="remark" placeholder="请在此处添加说明,可不填" v-model="remark">
        </li>
     </ul>
     <!--上传凭证-->
    <div class="uploadImg">
    ...
    <div class="gearArea" v-show="apply">
        <div class="applyReason">
            <ul v-for="(item, index) in reasonDetail" class="clearfix" :key="index">
                <li class="choose"><input type="radio" name="reason" v-model="reason" :value="item"><span>{{item}}</span></li>
            </ul>
    ...
    <alert-c :title="titleC" v-show="isShowC"></alert-c>
  </section>
</template>
...
<script> 
    import alertC from '../../components/alertD.vue'
    ...
    export default{
        ...
        methods:{
            ...
            submits(){
                ...
                this.titleC = "正在提交..."
                this.isShowC = !this.isShowC;               
                this.$http.get('/eb-wxshop/wxshop/order/saveOrderAfterSales',{params:{refundInfos:jsonData, feedbackimg1:this.imgString[0], feedbackimg2: this.imgString[1], feedbackimg3: this.imgString[2]}}).then((res)=>{
                    if(res.body.result == "000"){
                        this.titleC = '提交完成';
                    }else{                  
                        this.titleC = "提交失败"
